...
r.sendline(b'40')
payload = b'AAAAAAAA'+b'AAAAAAAA'+b'AAAAAAAA'+b'AAAAAApico'
...
r.interactive()
```
All of the `r.sendline`'s are for selecting options in the interface. As for the actual payload, it consists of an initial 24 bytes (8 blocks of A's), and then a final block of 6 A's, and "pico"- for a total of 34 bytes. The reason we are inputting 34 bytes, rather than 35 (which was the size of the original `object` struct, is because the original `object` struct accounted for a [null byte](https://null-byte.wonderhowto.com/newest/).
